...
/* Flash Protocol Functions */ // - all of these?
XStatus InitializeFlash(void);
int WriteFlash(u32 Address, u8 *Data, u32 Length);
...
XStatus Flash_Init(XQspiPs *QspiPtr, uint16_t DeviceId);
...
/* Flash Commands and Configuration */
...
// Function to initialize your flash interface
XStatus InitializeFlash(void) {
    XStatus Status;
    ...
    // Get the configuration for QSPI0 (typical device ID is 0)
    ...
    // Now your InstancePtr would be &QspiInstance when calling QSPI functions
    Status = XQspiPs_SetSlaveSelect(&QspiInstance); // Select first slave
    
    return Status;
}

// Example flash write function
int WriteFlash(u32 Address, u8 *Data, u32 Length) {
    // Use the address parameter
    xil_printf("Writing to address: 0x%08X\n", Address);
    return XQspiPs_PolledTransfer(&QspiInstance, Data, NULL, Length);
}
...
{
	uint32_t options = XQspiPs_GetOptions(InstancePtr);

	// Add the clock active low setting to options
	options |= XQSPIPS_CLK_ACTIVE_LOW_OPTION;
...
XStatus PLD_QSPI_SetClockPhaseOne(PLD_QSPI_t * InstancePtr)
{
	...
	// Add the clock phase 1 setting to options
	options |= XQSPIPS_CLK_PHASE_1_OPTION;
...
XStatus PLD_QSPI_SetCsManualToggle(PLD_QSPI_t * InstancePtr)
{
	...
	// Remove manual slave select toggle from options
...
//     xil_printf("QSPI Flash Memory Test Application\r\n");

//     /*
//      * Run the Qspi Flash example first
//      */
//     xil_printf("Running QSPI Greater than 128Mb Flash Test...\r\n");
...
// #endif
//     if (Status != XST_SUCCESS) {
//         xil_printf("QSPI Greater than 128Mb Flash Test Failed\r\n");
//         cleanup_platform();
//         return XST_FAILURE;
//     }
//     xil_printf("QSPI Greater than 128Mb Flash Test Passed\r\n\n");

//     /*
//      * Now run the custom flash test
//      */
//     xil_printf("Running Custom Flash Test...\r\n");
...
//     #endif
//     // only need base address
//     if (Status != XST_SUCCESS) {
//         xil_printf("Failed to initialize QSPI driver\r\n");
//         cleanup_platform();
//         return XST_FAILURE;
//     }
...
#endif
{
	...
	for (UniqueValue = UNIQUE_VALUE, Count = 0;
			(u32)Count < Flash_Config_Table[FCTIndex].PageSize;
			Count++, UniqueValue++) {
	 ...
	 */
	for (UniqueValue = UNIQUE_VALUE, Count = 0; Count < (int)MaxData;
	     Count++, UniqueValue++) {
...
void DieErase(XQspiPs *QspiPtr, u8 *WriteBfrPtr)
{
	...
	for(u32 DieCnt = 0; DieCnt < Flash_Config_Table[FCTIndex].NumDie; DieCnt++) {
		/*
...
/*******************************************************************************
*   Flash Protocol Initialization (make life easier by throwing this in)
*******************************************************************************/
XStatus Flash_Init(XQspiPs *QspiPtr, uint16_t DeviceId) {
    XStatus Status;
    XQspiPs_Config *QspiConfig;
    ...
    XQspiPs_SetOptions(QspiPtr, XQSPIPS_FORCE_SSELECT_OPTION |
                               XQSPIPS_MANUAL_START_OPTION |
                               XQSPIPS_HOLD_B_DRIVE_OPTION);

    XQspiPs_SetClkPrescaler(QspiPtr, XQSPIPS_CLK_PRESCALE_8);
    XQspiPs_Enable(QspiPtr);
